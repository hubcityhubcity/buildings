//+------------------------------------------------------------------+
//| Fortress FX - DONCHIAN LOCKED PHONE TEST                          |
//| Backtest-only build: no strategy preset required.                 |
//| This file is hard-locked to Donchian Breakout logic.              |
//+------------------------------------------------------------------+
...
input int    MAGIC_NUMBER                   = 202607092;
...
int OnInit()
{
   ...
   {
      Print("Fortress FX DONCHIAN LOCKED failed to create indicator handles.");
      return(INIT_FAILED);
   }

   Print("FORTRESS FX DONCHIAN LOCKED TEST initialized on ", g_symbol,
         ". Strategy=DONCHIAN_ONLY, EXECUTION_ENABLED=", EXECUTION_ENABLED,
         ", Lookback=", DONCHIAN_LOOKBACK_BARS,
         ", EMA bias=", DONCHIAN_REQUIRE_EMA_BIAS);
...
void OnTradeTransaction(const MqlTradeTransaction &trans, const MqlTradeRequest &request, const MqlTradeResult &result)
{
   ...
   if(net_profit < 0.0)
   {
      ...
      Print("DONCHIAN closed losing trade. Consecutive losses=", g_consecutive_losses, ", net=", DoubleToString(net_profit, 2));
      ...
      if(USE_LOSS_STREAK_COOLDOWN && MAX_CONSECUTIVE_LOSSES > 0 && g_consecutive_losses >= MAX_CONSECUTIVE_LOSSES)
      {
         ...
         Print("DONCHIAN loss-streak cooldown active until ", TimeToString(g_loss_cooldown_until, TIME_DATE|TIME_MINUTES), ".");
      }
   }
   else if(net_profit > 0.0)
   {
      if(g_consecutive_losses > 0)
         Print("DONCHIAN closed winning trade. Consecutive loss counter reset.");
...
void OnTick()
{
   ...
   if(!EXECUTION_ENABLED)
   {
      ...
      g_day_signal_count++;
      Print("DONCHIAN signal found but execution disabled: ", signal.direction,
...
//+------------------------------------------------------------------+
bool BuildDonchianSignal(TradeSignal &signal)
{
   ...
   {
      buy_setup = buy_setup && ema_fast[0] > ema_slow[0];
      sell_setup = sell_setup && ema_fast[0] < ema_slow[0];
   }
   ...
   if(buy_setup)
      return SetAtrSignal(signal, "BUY", ask, atr[0]);
...
void ExecuteSignal(TradeSignal &signal)
{
   bool sent = false;
   if(signal.direction == "BUY")
      sent = trade.Buy(signal.lots, g_symbol, 0.0, signal.stop_loss, signal.take_profit, "Fortress FX DONCHIAN BUY");
   else if(signal.direction == "SELL")
      sent = trade.Sell(signal.lots, g_symbol, 0.0, signal.stop_loss, signal.take_profit, "Fortress FX DONCHIAN SELL");

   if(sent)
   {
      g_last_trade_time = TimeCurrent();
      g_day_signal_count++;
      Print("DONCHIAN order sent: ", signal.direction,
            " lots=", signal.lots,
            " sl=", signal.stop_loss,
   ...
   {
      Print("DONCHIAN order rejected: ", trade.ResultRetcode(), " ", trade.ResultRetcodeDescription());
   }
}
...
bool LossCooldownActive()
{
   ...
   if(TimeCurrent() >= g_loss_cooldown_until)
   {
      ...
      g_consecutive_losses = 0;
      Print("DONCHIAN loss-streak cooldown cleared.");
      return false;
   }
...
void LockBot(string reason)
{
   ...
   Print("FORTRESS FX DONCHIAN LOCKED: ", reason);
}

...
int OnInit()
{
   ...
   Print("Fortress FX initialized on ", g_symbol, ". EXECUTION_ENABLED=", EXECUTION_ENABLED, ", ENABLE_SIGNALS=", ENABLE_SIGNALS);
...
void OnTick()
{
   ...
   if(!EXECUTION_ENABLED)
   {
      ...
      Print("Signal found but execution disabled: ", signal.direction, " entry=", DoubleToString(signal.entry, _Digits), " sl=", DoubleToString(signal.stop_loss, _Digits), " tp=", DoubleToString(signal.take_profit, _Digits), " lots=", DoubleToString(signal.lots, 2));
      return;
   }
...
bool BuildSignal(TradeSignal &signal)
{
   ...
   if(CopyRates(g_symbol, PERIOD_CURRENT, 1, 2, rates) < 2) return false;
